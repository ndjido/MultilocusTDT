...
for (phenotype in 0:0){
    for (village1 in 1:2){
        for (village2 in village1:2){
            ...
            ped=read.table(paste(path,"/MultilocusTDT/Data/CheikhSNPs_2013_02_28.txt",sep=""), sep="\t", header=TRUE)
            ...
            write.table(gendata2, file=paste(path,"/MultilocusTDT/Data/TempData/gendata2.txt",sep=""), sep=" ", quote=F, row.names=F, col.names=F)
            gendata2=read.table(paste(path,"/MultilocusTDT/Data/TempData/gendata2.txt",sep=""), sep=" ")
            ...
            write.csv2(ResultsMultilocus, file = paste(path,"/MultilocusTDT/Results/","weighted_res_multilocus_",gene,"_phen",phenotype,"_","village",paste(village1,village2,sep=""),".csv",sep=""), quote=F, row.names=F)
            write.table(round(transmat,1), file = paste(path,"/MultilocusTDT/Results/","weighted_transmat_",gene,"_phen",phenotype,"_","village",paste(village1,village2,sep=""),".txt",sep=""), sep="\t", quote=F)
            ...
            #p SimTestStat = matrix(NA,nbsimul,nrow(ResultsMultilocus))
            ...
            write.table(SimTestStat, file = paste(path,"/MultilocusTDT/Results/","weighted_SimTestStat_multilocus_",gene,"_phen",phenotype,"_","village",paste(village1,village2,sep=""),".txt",sep=""), quote=F, row.names=F, col.names=F)
